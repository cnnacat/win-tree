    ...
    log_node**       log_head)
{
    ...
    do
    {
            ...
            && wcscmp(L"..", file_name) != 0)
        {
            ...
            if (!new_file_node)
            {
                // Skip the file whose node fails to initialize
    ...
    while (head_node)
    {
        ...
        if (head_node->is_dir)
        {
            ...
            if (new_file_handle == INVALID_HANDLE_VALUE)
            {
                ...
                // Silently fail during runtime if anything happens to 
                // error when it comes to allocation of nodes
    ...
    wchar_t* argv[])
{
    ...
    {   
        ...
        if (!log_head)
        {
            ...
            if (!no_err)
            {
                wprintf(L"Failed to initalize log node; But no errors occured during the tree runtime");
                continue;
            }
            else
            {
